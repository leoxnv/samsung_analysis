...
##2.3 Read and bind activities codes form test and train 
...
##2.4 Read and bind subject info form test and train
sub_test<-read.table("test/subject_test.txt")
sub_train<-read.table("train/subject_train.txt")
subs<-rbind(sub_test, sub_train)
names(subs)<- c("subject")

##2.5 Bind data, activities and subject information.
fdb <- cbind(subs, acts, sets)

##2.6 Read activities names and merge wiht dataset
